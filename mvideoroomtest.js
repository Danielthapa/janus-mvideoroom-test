...
server = "http://34.83.95.233:8088/janus";

var apisecret =
	"NzQ5NmMxY2QwOTYyNGM1ODk2NDlkZWRjMDI0MTA3OGEuYzg5MWY3MDE3NmNkNDBlOTliZWY3YWUxNWJhODE3NzU=.9581c43342f8fc0803e4320e957a75719031ebbc55f8f776cdd7f1171bad575ab1ffa64a1d5e9717ce704009df76555f3ed5cd6e834b01655b5991d613c182f7";
var janus = null;
var sfutest = null;
			...
			if (!on) {
				...
				if (track.kind === "video" && feed) {
					...
					if (feed.remoteVideos === 0) {
						// No video, at least for now: show a placeholder
						if (
							$("#videoremote" + slot + " .no-video-container").length === 0
						) {
...
function addSimulcastButtons(feed, temporal) {
	...
	$("#sl" + index + "-0")
		...
		.click(function () {
			...
			toastr.info(
				"Switching simulcast substream (mid=" +
					mids[index] +
					"), wait for it... (lower quality)",
				null,
				{ timeOut: 2000 }
			);
			...
			$("#sl" + index + "-0")
				...
				.addClass("btn-info");
			remoteFeed.send({
				message: { request: "configure", mid: mids[index], substream: 0 },
			});
		});
	$("#sl" + index + "-1")
		.removeClass("btn-primary btn-success")
		...
		.click(function () {
			...
			if (!$("#sl" + index + "-0").hasClass("btn-success"))
				$("#sl" + index + "-0")
					...
					.addClass("btn-primary");
			remoteFeed.send({
				message: { request: "configure", mid: mids[index], substream: 1 },
			});
		});
	$("#sl" + index + "-2")
		.removeClass("btn-primary btn-success")
		...
		.click(function () {
			...
			if (!$("#sl" + index + "-0").hasClass("btn-success"))
				$("#sl" + index + "-0")
					...
					.addClass("btn-primary");
			remoteFeed.send({
				message: { request: "configure", mid: mids[index], substream: 2 },
			});
		});
	if (!temporal)
	...
	$("#tl" + index + "-0")
		...
		.click(function () {
			...
			toastr.info(
				"Capping simulcast temporal layer (mid=" +
					mids[index] +
					"), wait for it... (lowest FPS)",
				null,
				{ timeOut: 2000 }
			);
			...
			$("#tl" + index + "-0")
				...
				.addClass("btn-info");
			remoteFeed.send({
				message: { request: "configure", mid: mids[index], temporal: 0 },
			});
		});
	$("#tl" + index + "-1")
		.removeClass("btn-primary btn-success")
		...
		.click(function () {
			...
			if (!$("#tl" + index + "-0").hasClass("btn-success"))
				$("#tl" + index + "-0")
					...
					.addClass("btn-primary");
			remoteFeed.send({
				message: { request: "configure", mid: mids[index], temporal: 1 },
			});
		});
	$("#tl" + index + "-2")
		.removeClass("btn-primary btn-success")
		...
		.click(function () {
			...
			if (!$("#tl" + index + "-0").hasClass("btn-success"))
				$("#tl" + index + "-0")
					...
					.addClass("btn-primary");
			remoteFeed.send({
				message: { request: "configure", mid: mids[index], temporal: 2 },
			});
		});
}
